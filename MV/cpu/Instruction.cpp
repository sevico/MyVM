...
extern FILE* fp;
extern int debugHelp;
#include "../Disam/libdasm.h"
...
extern unsigned int MemoryStart;	//虚拟内存的地址
...
void * ParseRegAddr(bool w,char reg)
{
	switch(reg)
	{
	case 0:
...
void * FindSegAddr(bool w,char reg)
{
	reg&=0x03;
	switch(reg)
	{
...
void TranslateOpcExt(bool w,char** rg,char** rm)
{
...
void TranslateOpcExtSeg(bool w,char** rg,char** rm)
{
	unsigned short *seg,*off1,*off2;
	w=true;				//与Seg有关的操作，w只有是1
	unsigned int tds=tmpDs;
	tds<<=4;
...
bool Bit(void *BitBase, int BitOffset)
{
...
{
#ifdef _VM_DEBUG_
	fprintf(fDebug,"*********%4x:%4x%4x%4x*********** OpcError!\n",eCPU.cs,eCPU.ip,*(unsigned char *)(evIP+MemoryStart-1),*(unsigned char *)(evIP+MemoryStart));	
#endif
	char err[200];
	unsigned char *pc=(unsigned char *)MemoryStart+evIP-1;
	sprintf(err,"An unkown instruction [ %2x %2x %2x %2x %2x %2x ] was read at [ %4xh:%4xh ], MyVM only support 8086 instruction set in this version. MyVM will be terminated.",*(pc),*(pc+1),*(pc+2),*(pc+3),*(pc+4),*(pc+5),eCPU.cs,eCPU.ip);
	eVMerror(err);
}
void showSatic()
{
	char msg[200];
	sprintf(msg, "ax:%4xh", eCPU.ax);
...
{
#ifdef _VM_DEBUG_	
	if (*(unsigned char*)(evIP+MemoryStart)==0x13)// && eCPU.dl==0x80)
		__asm nop
	if (eCPU.dl==1)
		__asm nop
	if (bStartRecord && *(unsigned char*)(evIP+MemoryStart)!=0x10 && *(unsigned char*)(evIP+MemoryStart)<0x21)
		__asm nop
#endif
	LongCallNewIP(1);
	eCPU.sp-=2;
...
void IN_AL_N()
{
	if (bStartRecord)// && *(unsigned char*)(evIP+MemoryStart)!=0x92)
		__asm nop
...
void BSWAP()
{
	unsigned int temp;
	unsigned int *desc;
	desc=(unsigned int *)ParseRegAddr(1,*(unsigned char *)(eIMS-1));
	temp=*desc;
...
void func_0x6A(){
	char reg = *((char*)(evIP+MemoryStart));
	evIP += 1;
	char szTemp[256] = { 0, };
	switch (reg)
	{
	case 0:
		sprintf_s(szTemp, 200, "AX:%4x h", eCPU.ax);
		break;
	case 1:
		sprintf_s(szTemp, 200, "CX:%4x h", eCPU.cx);
		break;
	case 2:
		sprintf_s(szTemp, 200, "DX:%4x h", eCPU.dx);
		break;
	case 3:
		sprintf_s(szTemp, 200, "BX:%4x h", eCPU.bx);
		break;
	case 4:
		sprintf_s(szTemp, 200, "SP:%4x h", eCPU.sp);
		break;
	case 5:
		sprintf_s(szTemp, 200, "BP:%4x h", eCPU.bp);
		break;
	case 6:
		sprintf_s(szTemp, 200, "SI:%4x h", eCPU.si);
		break;
	case 7:
		sprintf_s(szTemp, 200, "DI:%4x h", eCPU.di);
		break;
	default:
		break;
	}
	if (strlen(szTemp)!=0){
		MessageBox(NULL, szTemp, NULL, 0);
...
void SetupInstructionTable()
{
	...
	InsTable[0x6A]=(unsigned int)func_0x6A;
